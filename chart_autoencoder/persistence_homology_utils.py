...
def compute_persistence_homology(charts, name=None):
    ...
    with mp.Pool(processes=num_processes) as pool:
        logging.info(f"Calculating persistence homology using {num_processes} processes")
        results = pool.map(
    ...
    if name is not None:
        ...
        for ax, (chart_id, persistence_diagram) in zip(axes, persistence_diagrams.items()):
